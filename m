Return-Path: <linux-sparse-owner@vger.kernel.org>
X-Original-To: lists+linux-sparse@lfdr.de
Delivered-To: lists+linux-sparse@lfdr.de
Received: from vger.kernel.org (vger.kernel.org [209.132.180.67])
	by mail.lfdr.de (Postfix) with ESMTP id DD204F203E
	for <lists+linux-sparse@lfdr.de>; Wed,  6 Nov 2019 21:59:34 +0100 (CET)
Received: (majordomo@vger.kernel.org) by vger.kernel.org via listexpand
        id S1728057AbfKFU7e (ORCPT <rfc822;lists+linux-sparse@lfdr.de>);
        Wed, 6 Nov 2019 15:59:34 -0500
Received: from antares.kleine-koenig.org ([94.130.110.236]:60540 "EHLO
        antares.kleine-koenig.org" rhost-flags-OK-OK-OK-OK) by vger.kernel.org
        with ESMTP id S1727351AbfKFU7d (ORCPT
        <rfc822;linux-sparse@vger.kernel.org>);
        Wed, 6 Nov 2019 15:59:33 -0500
Received: by antares.kleine-koenig.org (Postfix, from userid 1000)
        id 1F90882525B; Wed,  6 Nov 2019 21:59:30 +0100 (CET)
Date:   Wed, 6 Nov 2019 21:59:27 +0100
From:   Uwe =?iso-8859-1?Q?Kleine-K=F6nig?= <uwe@kleine-koenig.org>
To:     Luc Van Oostenryck <luc.vanoostenryck@gmail.com>,
        linux-sparse@vger.kernel.org
Subject: Re: Sparse v0.6.1
Message-ID: <42602ad0-05c2-b6a1-33ac-75191b80af88@kleine-koenig.org>
References: <20191013234630.7uuszc5auri46h4y@desk.local>
MIME-Version: 1.0
Content-Type: multipart/signed; micalg=pgp-sha512;
        protocol="application/pgp-signature"; boundary="J4XPiPrVK1ev6Sgr"
Content-Disposition: inline
In-Reply-To: <20191013234630.7uuszc5auri46h4y@desk.local>
Openpgp: preference=signencrypt
Autocrypt: addr=uwe@kleine-koenig.org; prefer-encrypt=mutual; keydata=
 
 kC8vBDJg
User-Agent: Mutt/1.10.1 (2018-07-13)
Sender: linux-sparse-owner@vger.kernel.org
Precedence: bulk
List-ID: <linux-sparse.vger.kernel.org>
X-Mailing-List: linux-sparse@vger.kernel.org


--J4XPiPrVK1ev6Sgr
Content-Type: text/plain; charset=us-ascii
Content-Disposition: inline

Hello Luc,

On 10/14/19 1:46 AM, Luc Van Oostenryck wrote:
> Sparse v0.6.1 is now out.

I uploaded sparse 0.6.1 to Debian and got two failed builds. Both mipsel
(https://buildd.debian.org/status/fetch.php?pkg=sparse&arch=mipsel&ver=0.6.1-1&stamp=1572991690&raw=0)
and powerpc
(https://buildd.debian.org/status/fetch.php?pkg=sparse&arch=powerpc&ver=0.6.1-1&stamp=1572991826&raw=0)
fail in the same way:

	  TEST    enum-sign-extend (enum-sign-extend.c)
		Using command       : sparse -m64 $file
		Expecting exit value: 0
	error: actual error text does not match expected error text.
	error: see enum-sign-extend.c.error.* for further investigation.
	--- enum-sign-extend.c.error.expected	2019-11-05 22:07:29.969247618 +0000
	+++ enum-sign-extend.c.error.got	2019-11-05 22:07:29.965247568 +0000
	@@ -0,0 +1 @@
	+enum-sign-extend.c:6:28: error: static assertion failed: "type"
	error: FAIL: test 'enum-sign-extend.c' failed

I didn't debug this any further.

Best regards
Uwe

--J4XPiPrVK1ev6Sgr
Content-Type: application/pgp-signature; name="signature.asc"

-----BEGIN PGP SIGNATURE-----


=fNTj
-----END PGP SIGNATURE-----

--J4XPiPrVK1ev6Sgr--
